...
class ExtractorTest < Minitest::Test
  ...
  def test_with_a_type_of_overwrite_or_dump_affecting_haml_writer
    h = I18n::Extractor.new(file_path("ex1.haml"), :type => :overwrite)
    assert_equal h.haml_writer.overwrite?, true
    h = I18n::Extractor.new(file_path("ex1.haml"))
    assert_equal h.haml_writer.overwrite?, false
  end

  def test_with_a_interactive_option_which_prompts_the_userper_line
    h = I18n::Extractor.new(file_path("ex1.haml"), :interactive => true)
  ...
  def test_with_a_interactive_option_user_can_tag_a_line_for_later_review
    ...
    if File.exist?(I18n::Extractor::TaggingWriter::DB)
      assert_equal File.readlines(I18n::Extractor::TaggingWriter::DB), []
    end
    h = I18n::Extractor.new(file_path("ex1.haml"), :interactive => true)
    user_input = "D" # dump
    File.readlines(file_path("ex1.haml")).size.times do
    ...
    assert (File.readlines(I18n::Extractor::TaggingWriter::DB).size != 0), "tag lines get added to file"
  end


  def test_can_not_initialize_if_the_haml_is_not_valid_syntax
    begin
      I18n::Extractor.new(file_path("bad.haml"))

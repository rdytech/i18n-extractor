...
class TextReplacerTest < Minitest::Test
  ...
  def test_it_initializes_with_the_line_it_is_going_to_replace_and_the_match_to_replace
    I18n::Extractor::TextReplacer.new("this is whatever", "this is whatever", :plain, "/path/to/doesntmatter.haml")
  end

  def test_but_it_raises_if_passed_a_wrong_line_type
    begin
      I18n::Extractor::TextReplacer.new("regular text", "regular text", :this_is_not_defined, "/path/to/doesntmatter.haml")
      assert false, 'should raise'
    rescue I18n::Extractor::NotDefinedLineType
      assert true, 'raised NotDefinedLineType'
    end
  end

  def test_it_wont_replace_if_whole_line_is_a_expression
    replacer = I18n::Extractor::TextReplacer.new('#{whole_line_expression}',
  ...
  def test_it_wont_replace_if_whole_line_in_tag_is_a_expression
    replacer = I18n::Extractor::TextReplacer.new('%span #{whole_line_expression}',
                                                        '#{whole_line_expression}',
  ...
  def test_it_wont_replace_attributes_if_replacement_place_is_content
    replacer = I18n::Extractor::TextReplacer.new('%span{title: "Height, cm"} Height',
                                                        'Height',
  ...
  def test_it_can_replace_title_attribute
    replacer = I18n::Extractor::TextReplacer.new('%span{title: "Height, cm"}= "Height, cm"',
                                                        'Height, cm',
                                                        :tag, '/path/to/doesntmatter.haml', {},
  ...
  def test_it_will_prepend_i18n_tag_with_filename_if_filename_prefix_option_passed
    replacer = I18n::Extractor::TextReplacer.new("this is whatever", "this is whatever", :plain, "path/to/doesntmatter.haml", {}, {
  ...
  def test_it_can_replace_the_body_of_haml_with_t_characters_example
    replacer = I18n::Extractor::TextReplacer.new("%span Admin Dashboard", "Admin Dashboard", :tag, "/path/to/doesntmatter.haml")
  ...
  def test_it_wont_replace_already_replaced_t_characters_if_they_are_not_ruby_evaled
    replacer = I18n::Extractor::TextReplacer.new("%span t('.admin_dashboard')", "t('.admin_dashboard')", :tag, "/path/to/doesntmatter.haml")
    assert_equal replacer.replace_hash, {:modified_line => "%span t('.admin_dashboard')",
  ...
  def test_it_wont_replace_already_replaced_t_characters_that_are_ruby_evaled
    replacer = I18n::Extractor::TextReplacer.new("%span= t('.admin_dashboard')", "t('.admin_dashboard')", :script, "/path/to/doesntmatter.haml")
    assert_equal replacer.replace_hash, {:modified_line => "%span= t('.admin_dashboard')",
                                          :t_name => "admin_dashboard", :replaced_text => "t('.admin_dashboard')", :path => "/path/to/doesntmatter.haml"}
  end
  ...
  def test_it_limits_the_characters_of_the_t_namespace_it_provides_to_limit_key_name
    replacer = I18n::Extractor::TextReplacer.new("this is whatever" * 80, "this is whatever" * 80, :plain, "/path/to/doesntmatter.haml")
    assert_equal I18n::Extractor::Helpers::StringHelpers::LIMIT_KEY_NAME + 9, replacer.replace_hash[:modified_line].size # = t('.')
  end

  def test_it_does_not_allow_weird_characters_in_the_keyname
    weird_line = "t<>hi:s = (is \"' `ch@ract.er ~ ma?dne{}s[]s!"
    replacer = I18n::Extractor::TextReplacer.new(weird_line, weird_line, :plain, "/path/to/doesntmatter.haml")

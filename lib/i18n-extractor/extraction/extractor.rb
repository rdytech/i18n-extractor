  ...
  class Extractor
    ...
    def initialize(haml_path, opts = {})
      ...
      if (@add_filename_prefix && !@base_path)
        ...
        @prompter.puts('e.g. i18n-extractor . --add-filename-prefix true --base-path /Users/jeremynagel/dev/some-project/views/')
        raise Error
      end

      @prompter = I18n::Extractor::Prompter.new
      @haml_reader = I18n::Extractor::HamlReader.new(haml_path)
      validate_haml(@haml_reader.body)
      @haml_writer = I18n::Extractor::HamlWriter.new(haml_path, {:type => @type})
      @yaml_writer = I18n::Extractor::YamlWriter.new(@options[:i18n_scope], @options[:yaml_file], {
          :haml_path => haml_path,
          :add_filename_prefix => opts[:add_filename_prefix],
          :base_path => opts[:base_path]
      })
      @tagging_writer ||= I18n::Extractor::TaggingWriter.new
    ...
    def process_line(orig_line, line_no)
      ...
      if user_action.tag?
        ...
        add_to_yaml_info(line_no, [I18n::Extractor::ReplacerResult.new(nil, nil, nil, false, nil).info])
        add_to_body(orig_line)
      end
    ...
    def replacement_result(orig_line, line_match, line_type, line_no, options)
      if line_match && !line_match.empty?
        result = I18n::Extractor::TextReplacer.new(orig_line, line_match, line_type, @haml_reader.path, line_metadata(line_no), {
        ...
        I18n::Extractor::ReplacerResult.new(orig_line, nil, line_match, false, "")
      end
    end
    ...
    def finding_result(orig_line, lineno)
      I18n::Extractor::TextFinder.new(orig_line, line_metadata(lineno)).process_by_regex
    end
    ...
    def user_action_yes
      I18n::Extractor::UserAction.new('y') # default if no prompting: just do it.
    end

    def user_action_no
      I18n::Extractor::UserAction.new('n') # don't replace
    end
